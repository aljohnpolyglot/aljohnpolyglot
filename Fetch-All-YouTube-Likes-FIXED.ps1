...
function Get-InfoCache {
    ...
    try {
        # Windows PowerShell 5.1 ConvertFrom-Json treats property names as
        # case-insensitive. Chrome's Local State can contain names that differ
        # only by case, which makes ConvertFrom-Json fail. JavaScriptSerializer
        # keeps those names distinct and is therefore safer for Chrome JSON.
        ...
        $raw = [IO.File]::ReadAllText($LocalStatePath)
        $local = $serializer.DeserializeObject($raw)
        ...
        if (
            ...
            foreach ($directory in $local['profile']['info_cache'].Keys) {
                $info = $local['profile']['info_cache'][$directory]
                ...
                if ($info -is [System.Collections.IDictionary]) {
                    if ($info.Contains('name'))      { $name = [string]$info['name'] }
                    if ($info.Contains('user_name')) { $userName = [string]$info['user_name'] }
                    if ($info.Contains('email'))     { $email = [string]$info['email'] }
                }

                $map[[string]$directory] = [pscustomobject]@{
                    name      = $name
                    user_name = $userName
                    email     = $email
                }
        ...
        # Profile resolution has a second, raw-text Preferences scan, so a
        # Local State parsing problem should not abort the whole automation.
        Write-Log "WARNING: Chrome Local State parser fallback: $($_.Exception.Message)"
    }

    # Ensure every real Chrome profile is represented. Resolve-ChromeProfileDirectory
    # will scan Preferences/Secure Preferences directly for the requested email.
