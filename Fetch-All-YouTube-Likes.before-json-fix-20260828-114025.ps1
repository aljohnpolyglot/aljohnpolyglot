...
function Get-InfoCache {
    ...
    $local = Get-Content -Raw -LiteralPath $LocalStatePath | ConvertFrom-Json
    if ($null -eq $local.profile -or $null -eq $local.profile.info_cache) {
        return @{}
    }
    $map = @{}
    foreach ($prop in $local.profile.info_cache.PSObject.Properties) {
